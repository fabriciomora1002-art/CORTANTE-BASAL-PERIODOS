...
COLOR_FONDO_APP = "#F0F2F6"      # Plomo muy suave (Gris azulado)
COLOR_CARD = "rgba(255, 255, 255, 0.95)" # Blanco con ligera transparencia
COLOR_TEXTO_PRINCIPAL = "#262730" # Gris oscuro casi negro (Alta legibilidad)
COLOR_ACCENT = "#007BFF"         # Azul Ingenieria (Vigas/Resaltados)
COLOR_COLUMNA = "#4B5563"        # Gris Acero (Columnas)
COLOR_ALERT = "#DC3545"          # Rojo suave (Puntos criticos)

# Inyeccion CSS para forzar el estilo Claro y corregir visibilidad de letras
st.markdown(f"""
    <style>
    /* Forzar fondo general de la aplicacion */
    .stApp {{
    ...
    section[data-testid="stSidebar"] {{
        background-color: #E5E7EB; /* Plomo un poco mas oscuro que el fondo */
        color: {COLOR_TEXTO_PRINCIPAL};
    }}
    
    /* Corregir color de texto de las etiquetas (Labels) de los inputs */
    .stMarkdown, .stText, h1, h2, h3, h4, h5, h6, label {{
        color: {COLOR_TEXTO_PRINCIPAL} !important;
        font-family: 'Segoe UI', Helvetica, sans-serif;
    }}
    
    /* Contenedores de metricas (Numeros grandes) con fondo blanco */
    ...
    div[data-testid="stMetricLabel"] {{
        color: #6B7280 !important; /* Gris medio para el titulo de la metrica */
    }}
    ...
    /* Estilo para los graficos (bordes suaves) */
    figure {{
...
# ==============================================================================
# 2. FUNCIONES DE CALCULO (MOTOR MATEMATICO - SIN CAMBIOS)
# ==============================================================================
...
# ==============================================================================
# 3. INTERFAZ DE USUARIO (PANEL LATERAL CORREGIDO)
# ==============================================================================
...
if btn_calc:
    with st.spinner('Realizando calculo matricial...'):
        
        # --- CALCULO MATRICIAL (Backend) ---
        ...
        w2_valid = w2_sorted[w2_sorted > 1e-5]
        ...
        # ==============================================================================
        # 5. VISUALIZACION (ESTILO CLARO)
        # ==============================================================================
        
        tab_main, tab_data = st.tabs(["Panel de Resultados", "Datos Tecnicos"])
        
        with tab_main:
            # Metricas en tarjetas blancas limpias
            ...
            # --- Grafico 1: Estructura (Fondo Blanco) ---
            with col_g1:
                st.subheader("Discretizacion del Modelo")
                fig1, ax1 = plt.subplots(figsize=(6, 5))
                # Configuracion colores claros
            ...
            with col_g2:
                ...
                ax2.legend(facecolor='white', edgecolor='#D1D5DB')
                for spine in ax2.spines.values(): spine.set_edgecolor('#D1D5DB')
                st.pyplot(fig2)

        with tab_data:
            st.info("Desglose de calculos internos")
            c_d1, c_d2 = st.columns(2)
            c_d1.write(f"**Factores Sitio:** Fa={Fa:.3f}, Fv={Fv:.3f}")
            c_d1.write(f"**Parametros:** SDS={SDS:.3f}, SD1={SD1:.3f}")
            c_d2.write(f"**Masa Total:** {peso_total:.2f} Ton")
